...
export type GroupColorClasses = (typeof GROUP_COLOR_PALETTE)[number];

export function getGroupColorClasses(
  group: Pick<ExperimentGroup, "order">,
): GroupColorClasses {
  const index =
    (((group.order - 1) % GROUP_COLOR_PALETTE.length) + GROUP_COLOR_PALETTE.length) %
    GROUP_COLOR_PALETTE.length;
